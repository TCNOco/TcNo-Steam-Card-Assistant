...
function IndividualPage() {
	...
	jQuery('div.badge_card_to_collect_header').after('<br /><a href="javascript:OpenAllMarket()" style="display: inline-block;margin-left:10px" class="btn_grey_grey btn_medium"><span>Open all market links</span></a>');
...
function BadgeListPage() {
	...
	if (TcNoMulticraftNumbers.length > 0){
		// Add styles to head
		...
		jQuery("div.profile_xp_block").after(
			...
			'<div class="craft_all_block"><div style="float: left;"><span>TcNo Steam Card Assistant</span></div><div style="float:right;"><a class="badge_craft_button TcNoButton" style="padding:0px 10px;display:inline-block;" href="javascript:TcNoMulticraftOneOfEach()">Craft 1 of each</a></div></div>'
		);
		...
		console_info = ["%c TcNo Steam Card Assistant: %cDisplaying 'craft 1 of all' button that will craft: " + TcNoMulticraftNumbers.length + " badges", 'background: #222; color: white','background: #222; color: #bada55'];
	}else{
